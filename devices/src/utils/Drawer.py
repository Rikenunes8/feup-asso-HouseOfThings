...
class Drawer:
    ...
    def drawLight(self, connected, state, color=None):
        if color == None:
            color = LIGHT_ON if state else LIGHT_OFF
        self.screen.fill(BG_COLOR)
    ...
    def drawThermometer(self, temperature):
        ...
        self.drawText(text, TEXT_COLOR, 10, HEIGHT - self.font.size(text)[1] * 1.5)
        pygame.draw.rect(
            self.screen, TEXT_COLOR, (WIDTH / 2 - 11, HEIGHT / 2 - 101, 1, 202)
        )
        pygame.draw.rect(
            self.screen, TEXT_COLOR, (WIDTH / 2 + 10, HEIGHT / 2 - 101, 1, 202)
        )
        pygame.draw.rect(
            self.screen, TEXT_COLOR, (WIDTH / 2 - 11, HEIGHT / 2 - 101, 22, 1)
        )
        pygame.draw.rect(
            self.screen, TEXT_COLOR, (WIDTH / 2 - 11, HEIGHT / 2 + 100, 22, 1)
        )
        ...
        pygame.draw.rect(
            ...
            (
                WIDTH / 2 - 10,
                HEIGHT / 2 - 100 + (200 - (temperature + 10) / 60 * 200),
                20,
                (temperature + 10) / 60 * 200,
            ),
        )
